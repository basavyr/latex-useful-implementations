...
\begin{figure}
    ...
    \begin{tikzpicture}[every text node part/.style={align=center}]
        ...
        \node[rectangle, draw=black, thick, fill=red!10, inner sep=0.2cm,minimum width=12 cm] at (2,-1.8) (first-box) {\textbf{Maximized} short-range interaction \\ \\ $\mathcal{Q}_n+\mathscr{C}$ = \emph{repulsive}};
        
        \node[rectangle, draw=black, thick, fill=red!10, inner sep=0.2cm,minimum width=12 cm] (second-box) [below=of first-box] {\textbf{Minimal} energy (PES)};
        ...
        \node[rectangle, draw=black,xshift=6cm,yshift=2cm,fill=red!10, minimum width=4cm] (maximal-box) {\textbf{Maximal overlap} \\ between \\ {\color{blue}$\mathcal{D}\left(\mathcal{Q}_n\right)$} and {\color{black!60!green}$\mathcal{D}\left(\mathscr{C}\right)$}};
    ...
    \caption{The workflow of a quasi-particle with hole character $\mathcal{Q}_n$ in its coupling with a triaxial rotor $\mathscr{C}$. Each of the five `steps' represents a characteristic involved in the wobbling motion, as described in text. Namely, for $\mathcal{Q}_n$ coming from the bottom of a $j$-shell and coupling with the $s$-axis of the even-even core $\mathscr{C}$ \textbf{(1)} will maximize the density overlap \textbf{(2)}, which will minimize their interaction \textbf{(3)}, minimizing the total energy \textbf{(4)}, and finally stabilizing the triaxial structure \textbf{5}. The long $s$-axis has been ignored within the coupling.}
